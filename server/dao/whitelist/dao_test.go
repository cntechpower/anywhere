...
func TestWhiteList(t *testing.T) {
	...
	defer log.Close()
	db.Init("anywhere:anywhere@tcp(10.0.0.2:3306)/anywhere_test?charset=utf8mb4&parseTime=True&loc=Local&readTimeout=5s&timeout=5s",
		model.GetPersistModels(), model.GetTmpModels())
	_, err := db.MySQL.Exec("truncate table whitelist_deny_history")
	...
	_, err = db.MySQL.Exec("drop table whitelist_deny_history")
	assert.Equal(t, nil, err)
}
